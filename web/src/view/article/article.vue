      ...
      >
        ...
        >
          ...
          <el-date-picker
            ...
            :disabled-date="
              (time) =>
                searchInfo.endDate ? time.getTime() > searchInfo.endDate.getTime() : false
            "
          />
          —
          <el-date-picker
            ...
            :disabled-date="
              (time) =>
                searchInfo.startDate
                  ? time.getTime() < searchInfo.startDate.getTime()
                  : false
            "
          />
        </el-form-item>
      ...
      >
        ...
        >
          <template #default="scope">
            {{ filterDict(scope.row.status, ArticleOptions) }}
          </template>
        </el-table-column>
        <el-table-column
          align="left"
      ...
      <el-scrollbar height="550px">
        ...
        >
          ...
          <el-descriptions-item label="状态">
            {{ filterDict(formData.status, ArticleOptions) }}
          </el-descriptions-item>
        </el-descriptions>
      </el-scrollbar>
    </el-dialog>
...
console.log(useUserStore().userInfo.authorityId)
console.log('hello,world')
defineOptions({
  // name: 'Article',
})

const roleId = useUserStore().userInfo.authorityId
  ...
  date: [
    {
      validator: (rule, value, callback) => {
        if (searchInfo.value.startDate && !searchInfo.value.endDate) {
          callback(new Error('请填写结束日期'))
        } else if (!searchInfo.value.startDate && searchInfo.value.endDate) {
          callback(new Error('请填写开始日期'))
        } else if (
          searchInfo.value.startDate &&
          searchInfo.value.endDate &&
          (searchInfo.value.startDate.getTime() === searchInfo.value.endDate.getTime() ||
            searchInfo.value.startDate.getTime() > searchInfo.value.endDate.getTime())
        ) {
          callback(new Error('开始日期应当早于结束日期'))
        } else {
